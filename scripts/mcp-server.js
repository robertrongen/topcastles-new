...
const DATA_PATH = join(__dirname, '../new_app/src/assets/data/castles_enriched.json');

// Load once at startup
const ALL_CASTLES = JSON.parse(readFileSync(DATA_PATH, 'utf8'));
const BY_SCORE = [...ALL_CASTLES].sort((a, b) => (b.score_total ?? 0) - (a.score_total ?? 0));
...
server.tool(
  ...
  ({ castle_code }) => {
    const castle = ALL_CASTLES.find(c => c.castle_code === castle_code);
    if (!castle) {
      return {
