...
1. Source spreadsheet input:
   - Current source: `old_app/database/Topcastles export.xlsx`
...
| --- | --- | --- | --- |
| `old_app/database/Topcastles export.xlsx` | Data pipeline input | Source, currently legacy-located | Active input for `scripts/xlsx_to_json.py`; must move outside `old_app/`. |
...
| --- | --- | --- | --- |
| `old_app/database/Topcastles export.xlsx` | REQUIRED FOR ACTIVE PIPELINE | `scripts/xlsx_to_json.py` reads this path; `npm run data:convert` runs that script. | Move to a current source-data location. |
...
| `docs/setup.md` references to `old_app/database/Topcastles export.csv` | DEAD/STALE REFERENCE | Current command uses XLSX via `scripts/xlsx_to_json.py`; this doc still says CSV. | Update in a later docs cleanup or as part of extraction. |
...
| --- | --- | --- | --- | --- |
| `old_app/database/Topcastles export.xlsx` | Canonical active spreadsheet input for `npm run data:convert`. | `source-data/topcastles/Topcastles export.xlsx` | `scripts/xlsx_to_json.py`, `docs/pipeline.md`, `docs/pipeline-flow.md`, `docs/setup.md`, and any future source-data references. | Low |
...
1. Create `source-data/topcastles/` and copy the tracked XLSX source there.
2. Update `scripts/xlsx_to_json.py` to read `source-data/topcastles/Topcastles export.xlsx`.
3. Run `npm run data:convert`, then inspect generated JSON for unintended changes.
4. Decide whether the CSV export moves with the XLSX or remains archival-only in `old_app/database/`.
5. Update `docs/pipeline.md`, `docs/pipeline-flow.md`, and `docs/setup.md` to name the new source-data location.
6. Run the full pipeline commands and commit source-data plus generated-and-committed outputs.
7. Review `tests/test_php_baseline.py` separately: either keep it as archive validation, move it to an archive-test category, or retire it if old PHP validation is no longer part of active project quality gates.
8. Only after source data and tests are settled, consider moving or labeling the remaining `old_app/` tree as archival.

## Open Questions

- Should `old_app/database/Topcastles export.csv` remain a supporting source export after the XLSX moves, or should it stay with the archived PHP app?
